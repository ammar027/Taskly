...
const useDeviceOrientation = () => {
  ...
  useEffect(() => {
    const checkOrientation = async () => {
      ...
      setOrientation(orientationInfo === ScreenOrientation.Orientation.LANDSCAPE_LEFT || orientationInfo === ScreenOrientation.Orientation.LANDSCAPE_RIGHT ? 'LANDSCAPE' : 'PORTRAIT')
    }
          ...
          tabBar={({state, descriptors, navigation}) => (
            <View style={styles.sidebarContainer}>
              <View style={styles.appNameContainer}>
                <Image style={styles.image} source={require('@/icons/adaptive-icon.png')} contentFit="cover" />
              </View>
              ...
              {state.routes.map((route, index) => {
                ...
                return (
                  ...
                  >
                    <Ionicons name={tab.icon as any} size={24} color={isFocused ? themeColors.activeTintColor : themeColors.inactiveTintColor} style={styles.tabIcon} />
                    <Text style={[styles.tabLabel, isFocused ? styles.activeTabLabel : styles.inactiveTabLabel]}>{label}</Text>
                  </Pressable>
                )
              })}
            </View>
        ...
        {tabConfig.map(tab => (
          <Tabs.Screen
            ...
            options={{
              title: tab.title,
              headerTitle: tab.title,
              tabBarIcon: ({size, color}) => <Ionicons name={tab.icon as any} size={size} color={color} />
            }}
          />
        ))}
      </Tabs>
