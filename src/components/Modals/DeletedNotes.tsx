  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        onRequestClose={() => deleteStatus === 'idle' && setConfirmDeleteVisible(false)}
      >
          ...
          <View style={[styles.confirmModalContent, { backgroundColor: theme.cardBackground }]}>
            {deleteStatus === 'idle' && (
              <>
                ...
                <Text style={[styles.confirmModalMessage, { color: theme.subTextColor }]}>
                  This will permanently delete the note and cannot be undone. Are you sure?
                </Text>
            ...
            {deleteStatus === 'pending' && (
              <View style={styles.loadingContainer}>
                ...
                <Text style={[styles.loadingText, { color: theme.textColor, marginTop: 16 }]}>
                  Deleting note...
                </Text>
              </View>
            )}
            ...
            {deleteStatus === 'completed' && (
              <View style={styles.successContainer}>
                ...
                <Text style={[styles.successText, { color: theme.textColor, marginTop: 16 }]}>
...
const DeletedNotesModal = ({ visible, onClose, theme, noteService }) => {
  ...
  const loadDeletedNotes = useCallback(() => {
    ...
    try {
      ...
      const realmDeletedNotes = noteService.current.realm.objects('Note')
        .filtered('userId == $0 && isDeleted == true && (hardDeleted == null || hardDeleted == false)', noteService.current.userId)
  ...
  // Hard delete a note
  const handleHardDelete = useCallback(async (noteId) => {
  ...
  const handleEmptyTrash = useCallback(async () => {
    ...
    try {
      setProcessingHardDelete(true);
      const allDeletedNotes = noteService.current.realm.objects('Note')
        .filtered('userId == $0 && isDeleted == true && (hardDeleted == null || hardDeleted == false)', 
                noteService.current.userId);
  ...
  const renderItem = useCallback(({ item, index }) => (
    <DeletedNoteCard 
      ...
      theme={theme}
    />
  ), [handleRestoreNote, handleHardDelete, theme]);
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: theme.backgroundColor }]}>
        ...
        {isLoading || processingHardDelete ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={[styles.loadingText, { color: theme.subTextColor, marginTop: 16 }]}>
              {processingHardDelete ? 'Processing deletions...' : 'Loading notes...'}
            </Text>
          </View>
        ) : isEmpty ? (
          <View style={styles.emptyContainer}>
            <Ionicons 
              ...
              size={48} 
              color={theme.isDarkMode ? '#6b7280' : '#94A3B8'} 
            />
          ...
          <>
            <View style={styles.actionBar}>
              ...
              <Pressable 
                style={[styles.emptyTrashButton, { opacity: deletedNotes.length > 0 ? 1 : 0.5 }]} 
