...
export default function SyncService({ userId }) {
  ...
  const syncData = async () => {
    ...
    const actuallyOnline = currentConnectionState.isConnected && currentConnectionState.isInternetReachable !== false
    ...
    if (!actuallyOnline) {
      ...
      if (timeSinceLastAttempt > 60000) {
        // Only retry if more than 1 minute since last attempt
        ...
        const batchPromises = batchNotesData.map(async (noteData) => {
          ...
          if (noteData.hardDeleted === true) {
            try {
              ...
              const { error: deleteError } = await supabase.from("notes").delete().eq("id", noteData.id)
          ...
          try {
            ...
            const { data: remoteNote, error: fetchError } = await supabase.from("notes").select("updated_at").eq("id", noteData.id).single()
            ...
            // Upsert to Supabase
            const { error } = await supabase.from("notes").upsert(supabaseNoteData)
