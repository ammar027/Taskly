...
import {View, Text, StyleSheet, Modal, FlatList, Pressable, Platform, TouchableOpacity, TouchableWithoutFeedback, StatusBar} from 'react-native'
            ...
            <Animated.View style={[styles.modalContent, animatedContentStyle]}>
              ...
              <FlatList
                ...
                renderItem={({item, index}) => (
                  <Animated.View entering={FadeInUp.delay(index * 50).springify()} style={styles.categoryItemContainer}>
                    <Pressable
                      style={({pressed}) => [styles.categoryItem, {backgroundColor: getCategoryItemBackground(item.color)}, isDarkMode && {borderColor: `${item.color}30`}, selectedCategory?.id === item.id && styles.selectedCategoryItem, selectedCategory?.id === item.id && {borderColor: item.color}, pressed && styles.categoryItemPressed]}
